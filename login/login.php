<?php

session_start();
    $Notice="";
    include("../connection.php");

    if($_SERVER['REQUEST_METHOD'] == "POST")
    {
        //something was posted
        $Roll_No = $_POST['Roll_No'];
        $Password = $_POST['Password'];

        if(!empty($Roll_No) && !empty($Password))
        {

            //read from database
            $query = "select * from user_details where Roll_No = '$Roll_No'";
            $result = mysqli_query($conn, $query);

            if($result)
            {
                if($result && mysqli_num_rows($result) > 0)
                {

                    $user_data = mysqli_fetch_assoc($result);
                    
                    if(password_verify($Password,$user_data['Password']))
                    {

                        if($user_data['Designation'] === "Member"){
                            $_SESSION['Roll_No'] = $user_data['Roll_No'];
                            header("Location: ../Main Page/mainpage.php",true);
                            die;
                        }else{
                            $Notice="You're no Member";
                        }
                    }else{
                        $Notice="Bad Credentials!";
                    }
                }
                else
                {
                    $Notice="Credentials doesn't exist";
                }
            }
        }
    }
?>

<html>
    <head>
        <title>Delta Member Login</title>
        <link rel="stylesheet" href="login.css">
        <link rel="preconnect" href="https://fonts.googleapis.com">
        <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
        <link href="https://fonts.googleapis.com/css2?family=Carter+One&display=swap" rel="stylesheet">
        <link rel="icon" href="data:image/png;base64," type="image/x-icon">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <script src="login.js"></script>
    </head>
    <body>
        <img class="icon" src="data:image/png;base64," alt="Delta">
        <section class="welcome">
            <div class="login">
                <label class="Head"><span style="color: limegreen;">L</span>ogin</label>
                <label style="font-size: 15px;margin-left: 2%;color:crimson;position:absolute;"><?php echo $Notice;?></label>
                <form action="" method="POST" autocomplete="off">
                    <div class="logindiv">
                        <label><span style="color: limegreen;">R</span>oll No.</label>
                        <i class="fa fa-user-circle" style="font-size: 30px;margin-left: 71%;"></i>
                        <input class="loginin" name="Roll_No" type="number" min="0" autofocus required/>
                        <label><span style="color: limegreen;">P</span>assword</label>
                        <i class="fa fa-lock" style="font-size: 30px;margin-left: 68%;"></i>
                        <input class="loginin" name="Password" type="password" maxlength="16" minlength="8" required/>
                        <input class="loginbutton" name="submit" type="submit"/>
                    </div>
                </form>
                <label class="regis" id="regis"><span style="color: limegreen;">N</span>ew Member? <a href="..\register\register.php" style="color: red;cursor: pointer;">Register</a></label>
                <div class="loginback" onclick="window.location.assign('../welcome.html')">
                    <i class="fa fa-angle-left"></i>
                    <label style="cursor: pointer;">Go Back</label>
                </div>
            </div>
        </section>
    </body>
</html>