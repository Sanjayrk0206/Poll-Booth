...
<html>
    <head>
        ...
        <link rel="stylesheet" href="formpoll.css">
    </head>
    <body>
        <section>
            <h1><?php echo $Name;?></h1>
            <form action="savepolldata.php?Pname=<?php echo $Name;?>" method="POST" autocomplete="off">
                <table class="list">
                    <?php foreach($column as $list) {$list=str_replace("_"," ",$list);?>
                    <tr>
                        <td><label><?php echo $list;?></label><br>
                        <?php if($list=='Roll No') {?>
                            <input type="text" name="data[]" pattern="[0-9]+" value="<?php echo $Roll_No?>" readonly/>
                        <?php }else{ ?>
